...
def find_gradients(cost, kernels, weights):
    grads_k = []
    grads_w = []
    for kernel in kernels:
        grads_k.extend(tf.gradients(xs = kernel, ys = cost))

    for weight in weights:
        grads_w.extend(tf.gradients(xs = weight, ys = cost))

    return grads_k, grads_w

def update_gradients(learning_rate, kernels, grads_k, weights, grads_w):
    update_grads = []
    for W, dW in zip(weights, grads_w):
        a = W.assign(W - learning_rate * dW)
        update_grads.append(a)

    for K, dK in zip(kernels, grads_k):
        a = K.assign(K - learning_rate * dK)
        update_grads.append(a)
...
def model(X_train, Y_train, X_test, Y_test, epoches = 10, learning_rate = 1e-4, batch_size = 100, weight_size = 3, weight_channels = 128, no_cnn_layers = 10):
    ...
    A, kernels, weights = feed_forward(X, no_cnn_layers, weight_size, weight_channels, 100, 10)

    cost = compute_cost(A, Y)
    
    grads_k, grads_w = find_gradients(cost, kernels, weights)
    update_grads = update_gradients(learning_rate, kernels, grads_k, weights, grads_w)
